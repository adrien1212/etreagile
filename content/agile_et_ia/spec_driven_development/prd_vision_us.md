...
Si [écrire du code devient bon marché]({{< relref "agile_et_ia/spec_driven_development/index" >}}), alors la qualité du produit ne dépend plus de la qualité du code : elle dépend de la **qualité de ce qu'on a demandé**. Le livrable de l'humain devient la spécification.
...
Les outils SDD réintroduisent une hiérarchie que nous connaissons déjà sous le nom d'[oignon de la planification]({{< relref "planifier/niveau_planification/index" >}}) :

| Niveau | Artefact SDD | Question à laquelle il répond |
| --- | --- | --- |
| [Vision]({{< relref "planifier/niveau_planification/vision" >}}) | `constitution`, principes du projet | Pourquoi ce produit existe, ce qu'on ne fera jamais |
...
Le mouvement est le même que dans un [Product Backlog]({{< relref "framework_scrum/artefacts/product_backlog" >}}) [affiné progressivement]({{< relref "planifier/niveau_planification/index" >}}) : on part du vague pour aller vers le précis. Ce qui change, c'est qu'**à chaque niveau, ce qui n'est pas écrit n'existe pas**.
...
Toute notre approche des [récits utilisateur]({{< relref "user_stories/index" >}}) repose sur une thèse forte de Jeff Patton et Ron Jeffries : la user story **n'est pas une spécification**. Les [3C]({{< relref "user_stories/user_stories_mapping/les_3c" >}}) posent que la [carte]({{< relref "user_stories/user_stories_mapping/les_3c#la-carte" >}}) est volontairement pauvre, qu'elle n'est qu'un *support pour se rappeler de la conversation*, et que la compréhension naît de la [Conversation]({{< relref "user_stories/user_stories_mapping/les_3c#la-conversation" >}}) :
...
Autrement dit, le déplacement se fait à l'intérieur des 3C, de la Carte vers la [Confirmation]({{< relref "user_stories/user_stories_mapping/les_3c#la-confirmation" >}}) : la liste de critères d'acceptation cesse d'être un aide-mémoire pour devenir le **contrat de vérification** du travail de l'agent. C'est aussi ce qui relie cette page à notre réflexion sur la [documentation vivante]({{< relref "reflexion/pas_de_documentation" >}}) : la Confirmation automatisée *est* la documentation.

## Pourquoi INVEST se durcit

[INVEST]({{< relref "user_stories/rediger_us/stories_invest" >}}) était une grille de bonnes pratiques : une story qui la respectait mal se rattrapait en réunion. Ce filet disparaît. Reprenons les six critères.

**[I]({{< relref "user_stories/rediger_us/stories_invest#indépendante" >}}) — Indépendante : le critère le plus menacé.** Un LLM optimise la cohérence technique, pas la livraison incrémentale : livré à lui-même, il produit un découpage **horizontal** (une couche technique partagée d'abord, les stories ensuite) plutôt que **vertical**. Nous l'avons observé en direct dans [l'exemple SDD]({{< relref "agile_et_ia/spec_driven_development/exemple_sdd" >}}), avec une phase *Foundational* explicitement bloquante. Le rôle humain est de **contester ce découpage**.

**[N]({{< relref "user_stories/rediger_us/stories_invest#négociable" >}}) — Négociable : le critère qui perd son interlocuteur.** Une story négociable présuppose quelqu'un avec qui négocier. Un agent ne négocie pas : il implémente ce qui est écrit, y compris une bêtise. La négociation doit donc avoir lieu **avant l'écriture**, entre humains. Le danger est net : si personne ne l'assume, la story redevient un **contrat**, et nous retombons dans la spécification-contrat que le Manifeste combattait.

**[V]({{< relref "user_stories/rediger_us/stories_invest#valuable" >}}) — Valuable : le critère le plus souvent trahi par les outils.** Voir la section suivante.

**[E]({{< relref "user_stories/rediger_us/stories_invest#estimable" >}}) — Estimable : le critère qui change de sens.** Estimer l'effort *humain* d'une story perd sa signification quand c'est un agent qui l'implémente. Ce qui reste utile, c'est l'usage qu'en fait le [débat No Estimates]({{< relref "reflexion/estimer" >}}) : *si l'équipe n'arrive pas à estimer, c'est que la story est trop grosse ou mal comprise*. L'estimation devient un **détecteur de flou**, plus qu'une prévision.

**[S]({{< relref "user_stories/rediger_us/stories_invest#small" >}}) — Small : le critère qui protège du tunnel.** C'est la taille du lot qui distingue le SDD d'un [retour au Waterfall]({{< relref "methodologie_waterfall/definition" >}}). Rien n'empêche techniquement de demander à une IA un PRD couvrant six mois — sauf la discipline.

**[T]({{< relref "user_stories/rediger_us/stories_invest#testable" >}}) — Testable : le critère qui devient le mécanisme de contrôle.** C'est le seul moyen de vérifier le travail de l'agent sans relire chaque ligne, d'où l'importance renouvelée du [TDD]({{< relref "framework_xp/circle_of_life" >}}) et d'une [Definition of Done]({{< relref "framework_scrum/artefact_engagements/definition_of_done" >}}) explicite. Une story non testable n'est plus « perfectible » : elle est **invérifiable**.
...
> [!affirmation] Affirmation
> Le test le plus rapide reste celui de la [valeur]({{< relref "philosophie_agile/pourquoi/produire_valeur" >}}) : si personne ne peut expliquer à qui la story profite, ce n'est pas une story.
...
C'est ce qui rend le [User Story Mapping]({{< relref "user_stories/user_stories_mapping/index" >}}) plus utile que jamais : il part des utilisateurs et de leurs activités, donc il **impose** l'ancrage dans la valeur d'usage, là où une génération descendante depuis un PRD technique produit des tâches déguisées.
...
- **Mesurer l'[outcome]({{< relref "agile/output_vs_outcome" >}}).** Si produire est bon marché, livrer beaucoup ne prouve plus rien.
