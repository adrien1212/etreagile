...
Je vous propose ci-dessous de regarder un projet SDD x IA, mené avec [GitHub Spec Kit]({{< relref "ouvertures/agile_et_ia/spec_driven_development/index" >}}). L'intérêt de cet exemple est de voir **les artefacts réellement produits**, et non le discours qui les entoure.
...
On reconnaît au passage deux pratiques que nous avons déjà croisées : `contracts/` relève du *design-first* d'API (écrire le contrat avant le code, comme dans [l'exemple C4]({{< relref "ouvertures/urbanisation/c4_model/exemple" >}})), et `checklists/` joue le rôle d'une [Definition of Done]({{< relref "frameworks/framework_scrum/artefact_engagements/definition_of_done" >}}).
...
- **`[P]`** : la tâche peut être menée **en parallèle** (fichiers différents, pas de dépendance). C'est une déclaration explicite des [dépendances]({{< relref "equipe_agile/constitution_equipe/equipe_independante/dependance_equipe" >}}) — sauf qu'ici, elle sert à savoir combien d'agents peuvent travailler simultanément.
...
> Ce fichier est un [Sprint Backlog]({{< relref "frameworks/framework_scrum/artefacts/sprint_backlog" >}}). Nous écrivions que *dans le Product Backlog on liste les fonctionnalités, alors que dans le Sprint Backlog on liste les activités correspondant aux fonctionnalités à implémenter*. C'est exactement la relation entre `spec.md` et `tasks.md`.
...
Enfin, les tâches accomplies sont cochées (`[X]`) directement dans le fichier : le `tasks.md` fait office de [management visuel]({{< relref "pratiques/pilotage/management_visuel" >}}) pour l'agent comme pour l'humain, et rend visible l'avancement réel.
